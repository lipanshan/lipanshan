$(function (){
	...
	var tp = '';//开发者类型
	var type = 0;//应用类型
	var pages = 5;//每一页展示多少个app
	var nubPages = 5;//显示页码的个数
	var pageNum = 1;//页码
	var nowPageNum = 1; //当前页码
	var maxNumPage = 1;//最大页码
	var addPageNum = 1;
	var onLinePageNum = 1;
	var auditPassPageNum = 1;
	var offLinePageNum = 1;
	var waitForAuditPageNum = 1;
	var auditNotPassPageNum = 1;
		// 点击线上管理跳到线上管理界面
		$('#lyygl-list').click(function (ev){
			if(ev.target.innerHTML == '线上管理'){
				...
				// ajax请求数据开始
				var datas = 'proId='+index.split('&')[0]+'&type='+index.split('&')[1]+'&page='+nowPageNum+'&pageSize='+pages+addString;
				prId = index.split('&')[0];
				tp = index.split('&')[1];
				$.ajax({
					...
					success:function (data){
						var ret = JSON.parse(data);
						if(ret.success){
							var s = allApp(ret.apps);
							$('#lxsgl-applist').html(s);
							$('#lyygl-qbnum').html(ret.all);
							$('#lyygl-ysx .lcount').html(ret.online);
							$('#lyygl-shtg .lcount').html(0);
							$('#lyygl-yxx .lcount').html(ret.offline);
							$('#lyygl-ddsh .lcount').html(ret.audit0);
							$('#lyygl-shwtg .lcount').html(ret.audit2);
							maxNumPage = Math.ceil(ret.all/pages);
							creatPageNum(1,nubPages+1,maxNumPage);
						}else {
							alert(ret.errMsg);
							logintimeout(ret);
						}
		...
		$('#lyyzt-list').children().bind('click',function (){
				$(this).children().addClass('activeLeft');
				$(this).children().children().addClass('activeRight');
				$(this).siblings().children().removeClass('activeLeft');
				$(this).siblings().children().children().removeClass('activeRight');
		});
		// 分页开始
		// 点击全部按钮显示全部应用
		...
		function allFn(data){
			$.ajax({
				...
				success:function (data){
					var ret = JSON.parse(data);
					if(ret.success){
						$('#lxsgl-applist').html(allApp(ret.apps));
						type = 0;
					}else {
						alert(ret.errMsg);
						logintimeout(ret);
		...
		$('#lyygl-qb').click(function (){
			nowPageNum = addPageNum;
			maxNumPage = Math.ceil(Number($('#lyygl-qbnum').html()))/pages+1;
			var datas = 'proId='+prId+'&type='+tp+'&page='+nowPageNum+'&pageSize='+pages+addString;
			allFn(datas);
			creatPageNum(nowPageNum,nowPageNum+nubPages,maxNumPage);
		});
		// 点击已上线标签
		$('#lyygl-ysx').click(function (){
			nowPageNum = onLinePageNum;
			maxNumPage = Math.ceil(Number($('#lyygl-ysx').children('span').html()))/pages+1;
			type = 1;
			requestAppData(prId,tp,nowPageNum,pages,1);
			creatPageNum(nowPageNum,nowPageNum+nubPages,maxNumPage);
		});
		// 点击审核通过的标签
		$('#lyygl-shtg').click(function (){
			// nowPageNum = auditPassPageNum;
		});
		// 点击已下线标签
		$('#lyygl-yxx').click(function (){
			nowPageNum = offLinePageNum;
			maxNumPage = Math.ceil(Number($('#lyygl-yxx').children('span').html()))/pages+1;
			type = 4;
			requestAppData(prId,tp,nowPageNum,pages,4);
			creatPageNum(nowPageNum,nowPageNum+nubPages,maxNumPage);
		});
		// 点击等待审核标签
		$('#lyygl-ddsh').click(function (){
			nowPageNum = waitForAuditPageNum;
			maxNumPage = Math.ceil(Number($('#lyygl-ddsh').children('span').html()))/pages+1;
			type = 2;
			requestAppData(prId,tp,nowPageNum,pages,2);
			creatPageNum(nowPageNum,nowPageNum+nubPages,maxNumPage);
		});
		// 点击审核未通过标签
		$('#lyygl-shwtg').click(function (){
			nowPageNum = auditNotPassPageNum;
			maxNumPage = Math.ceil(Number($('#lyygl-shwtg').children('span').html()))/pages+1;
			type = 3;
			requestAppData(prId,tp,nowPageNum,pages,3);
			creatPageNum(nowPageNum,nowPageNum+nubPages,maxNumPage);
		});
				// 显示app信息函数
		function requestAppData(prId,types,pages,pagesSize,tabId){
			var datan ='proId='+prId+'&type='+types+'&page='+pages+'&pageSize='+pagesSize+'&tabId='+tabId+addString;
			$.ajax({
				url: '/EmindStoreManager/managerAction/appsByType',
				type: 'post',
				data: datan,
				success:function (data){
					var ret = JSON.parse(data);
		...
		// 创建页码函数
		function creatPageNum(startNum,endNum,maxNum){
			var strings = '';
			$('#lpages-box').show(0);
			if(isNaN(startNum)||isNaN(endNum)||isNaN(maxNum)){
				return;
			}
			if(Number(startNum)<=0||Number(endNum)<=0||Number(maxNum)<=0){
				$('#lpages-box').hide(0);
			}
			if(endNum >maxNum&&startNum<maxNum){
				endNum = maxNum;
			}else if(startNum>maxNum){
				return;
			}else if(maxNum == 1){
				startNum = 1;
				endNum = 2;
			...
			for(var i= startNum; i< endNum; i++){
				strings+='<a href="javascript:;">'+i+'</a>';

			}
			$('#lpagen').html(strings);
			for(var j = 0; j < $('#lpagen')[0].children.length;j++){
				if($('#lpagen')[0].children[j].innerHTML == nowPageNum){
					pageTag($('#lpagen')[0].children[j]);
				}
			}
		}
		// 页码添加样式
		...
		// 根据类型来保存进入了第几页
		function recordNum(itype){
			if(itype == 0){
				addPageNum = nowPageNum ;
			}else if(itype == 1){
				onLinePageNum = nowPageNum;
			}else if(itype == 2){
				waitForAuditPageNum = nowPageNum;
			}else if(itype == 3){
				auditNotPassPageNum = nowPageNum;
			}else if(itype == 4){
				offLinePageNum = nowPageNum;
			}
		}
			// 页码向左移、右移
		$('#lpagelist').click(function (ev){
			if(ev.target.innerHTML == '&lt;'){
				var firstNum = Number($('#lpagen').children().eq(0).html());
				if(!isNaN(firstNum)){
					if(firstNum>1){
						creatPageNum(firstNum-1,firstNum+nubPages-1,maxNumPage);
					}else {
						return;
					}
				}
			...
			if(ev.target.innerHTML == '&gt;'){
				var firstNum2 = Number($('#lpagen').children().eq(0).html());
				if(!isNaN(firstNum2)){
					if(maxNumPage>firstNum2+nubPages-1){
						creatPageNum(firstNum2+1,firstNum2+nubPages+1,maxNumPage);
					}else {
						return;
					}
				}
		...
		$('#lpagen').bind('click',function (ev){
			if(!isNaN(ev.target.innerHTML)){
				nowPageNum = Number(ev.target.innerHTML);
				if(type >=1&&type <=4){
					requestAppData(prId,tp,nowPageNum,pages,type);
					recordNum(type);
				}else if(type == 0){
					var datas = 'proId='+prId+'&type='+tp+'&page='+nowPageNum+'&pageSize='+pages+addString;
					recordNum(type);
		...
		$('#lpageBtn').click(function (){
			...
			if(!isNaN($('#linputPageNum').val())){

				var nx = parseInt($('#linputPageNum').val());
				if(nx>maxNumPage||nx<1){
					return;
				}else {
					nowPageNum = parseInt($('#linputPageNum').val());
				}
				if(type >=1&&type <=4){
					requestAppData(prId,tp,nowPageNum,pages,type);
				}else if(type == 0){
...
function allApp(index){
	...
	var strings = ysxArr.join('')+shtgArr.join('')+yxxArr.join('')+ddshArr.join('')+shwtgArr.join('');;
	return strings;
}
