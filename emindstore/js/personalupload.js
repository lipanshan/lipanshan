...
$(function (){
	...
	var uploaderApp = new plupload.Uploader({
		...
		init: {
			...
			UploadProgress: function(up, file) {
				// document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = '<span class>' + file.percent + "%</span>";
			...
			FileUploaded:function (up,file,res){
					...
					if(ret.success){
						...
						$('#packageName').html(ret.packageName);
				  		...
				  		// ''+'/EmindStoreManager/file/'+imgSrc[i].split('file')[1]+'';
				  		// var lsrc = '/EmindStoreManager/file/'+ ret.apklogo.split('file')[1]+'';
